...
public abstract class SQLiteOpenHelper {
     ...
     * @return a read/write database object valid until {@link #close} is called
     */
    public synchronized SQLiteDatabase getWritableDatabase(String password, SQLiteDatabaseHook hook) {
        if (mDatabase != null && mDatabase.isOpen() && !mDatabase.isReadOnly()) {
            return mDatabase;  // The database is already open for business
        }
        ...
        try {
            ...
            if (mName == null) {
                ...
                if (hook == null) {
                	...
                	db = SQLiteDatabase.openOrCreateDatabase(path, password, mFactory, hook);
                }
                //db = SQLiteDatabase.openDatabase(path,mFactory , SQLiteDatabase.OPEN_READWRITE);
                //db = mContext.openOrCreateDatabase(mName, 0, mFactory);
     ...
     *     or {@link #close} is called.
     */
    public synchronized SQLiteDatabase getReadableDatabase(String password, SQLiteDatabaseHook hook) {
        if (mDatabase != null && mDatabase.isOpen()) {
            return mDatabase;  // The database is already open for business
        }
        ...
        try {
            ...
            if(!databasePath.exists()){
                mIsInitializing = false;
                db = getWritableDatabase(password, hook);
                mIsInitializing = true;
                db.close();
            }
            if (hook == null) {
            	...
            	db = SQLiteDatabase.openDatabase(path, password, mFactory, SQLiteDatabase.OPEN_READONLY, hook);
            }
            if (db.getVersion() != mNewVersion) {
                throw new SQLiteException("Can't upgrade read-only database from version " +
                        db.getVersion() + " to " + mNewVersion + ": " + path);
